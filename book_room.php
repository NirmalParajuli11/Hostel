...
$userStmt = $conn->prepare("SELECT name, email, phone, address, food_preference, photo FROM users WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['room_id'])) {
    ...
    $stmt = $conn->prepare("SELECT r.id, r.room_type,
               CASE WHEN LOWER(r.room_type) = 'double' THEN 2
                    WHEN LOWER(r.room_type) = 'triple' THEN 3 ELSE 1 END AS total_beds,
...
$stmt = $conn->prepare("SELECT rb.id as booking_id, rb.created_at, rb.checkin_date, r.room_number, r.room_type, r.room_price 
    FROM room_bookings rb JOIN rooms r ON rb.room_id = r.id WHERE rb.user_id = ?");
$stmt->bind_param("i", $userId);
$stmt->execute();
